...
import {
  AlertCircle, AlertTriangle, Archive, Bot, CheckCircle2, CircleStop, Cpu,
  Database, Download, FileText, Gauge, History, LayoutDashboard, LockKeyhole,
  LoaderCircle, Mail, Network, Newspaper, Pause, PauseCircle, Play, RefreshCw, Save, Search, Send,
...
function RunStatusIcon({ status }: { status?: string }) {
  ...
  if (status === 'failed' || status === 'round_failed') return <AlertTriangle size={19} />
...
function Dashboard() {
  ...
  return <div className="page">
    <section className={`loop-banner status-${latest?.status || 'idle'}`}><div className="loop-state"><span className="pulse-icon"><RunStatusIcon status={latest?.status} /></span><div><strong>{latest ? `${latest.status === 'completed' ? '持续协作任务已完成' : latest.status === 'failed' ? '持续协作任务失败' : latest.status === 'paused' ? '持续协作任务已暂停' : `持续协作任务${latest.status === 'between_rounds' ? '等待下一轮' : '运行中'}`} · 第 ${latest.current_round || 1} 轮` : '持续协作任务空闲'}</strong><span>{latest ? `${latest.id} · ${stageLabels[latest.current_stage] || latest.current_stage}${latest.stop_after_round ? ` · 将在第 ${latest.stop_after_round} 轮结束` : ''}` : '从任务编排启动后，10 个标准 Agent 将协作完成全流程'}</span>{latest && <span className="runtime-meta">{latest.orchestrator_engine} · checkpoint {latest.checkpoint_backend} · 恢复 {latest.recovery_count || 0} 次</span>}</div></div><div className="actions">{latest && ['running', 'recovering', 'paused', 'between_rounds', 'stopping_after_round', 'needs_attention'].includes(latest.status) && <><Button onClick={() => action.mutate({ name: latest.status === 'paused' || latest.status === 'needs_attention' ? 'resume' : 'pause' })}>{latest.status === 'paused' || latest.status === 'needs_attention' ? <Play size={15} /> : <Pause size={15} />}{latest.status === 'paused' || latest.status === 'needs_attention' ? '恢复' : '暂停'}</Button>{latest.status !== 'needs_attention' && !latest.stop_after_round && <Button onClick={() => action.mutate({ name: 'stop-after-current-round' })}>本轮结束后停止</Button>}{latest.status !== 'needs_attention' && !latest.stop_after_round && <Button onClick={() => action.mutate({ name: 'stop-after-next-round' })}>再运行一轮后停止</Button>}<Button danger onClick={() => action.mutate({ name: 'cancel' })}><CircleStop size={15} />立即停止</Button></>}</div></section>
...
function Orchestrator({ user, onUserChange }: { user: User; onUserChange: (user: User) => void }) {
  ...
  const filtered = useMemo(() => (models.data?.items || []).filter((item) => item.id.toLowerCase().includes(modelSearch.toLowerCase())), [models.data?.items, modelSearch])
  ...
  return <div className="page"><div className="page-head"><div><h2>任务编排</h2><p>同一组标准 Agent 按轮协作执行完整流程，直到用户要求在轮次边界停止。</p></div><Button primary disabled={!selectedModel || start.isPending} onClick={() => start.mutate()}><Play size={15} />{start.isPending ? '启动中…' : '启动标准 Agent 协作'}</Button></div>
...
function Scenarios() {
  type ArticleItem = { id: number; title: string; subtitle: string; status: string; content_markdown: string; content_html: string; validation: { passed: boolean; errors: string[] }; word_count: number; assets: Array<{ id: number; file_path: string; mime_type: string; caption: string }> }
  type ArticleSettings = { enabled: boolean; generate_time: string; timezone: string; approval_required: boolean; auto_visuals: boolean; daily_article_count: number; article_model_id?: string | null; image_model_id?: string | null; image_count: number }
  const queryClient = useQueryClient(); const setNotice = useAppStore((state) => state.setNotice)
  const articles = useQuery<{ items: ArticleItem[] }>({ queryKey: ['articles'], queryFn: () => api('/api/articles'), refetchInterval: 5000 })
  const settingsQuery = useQuery<ArticleSettings>({ queryKey: ['article-settings'], queryFn: () => api('/api/article-settings') })
  const models = useQuery<{ items: Array<{ id: string }>; total: number }>({ queryKey: ['article-models'], queryFn: () => api('/api/models?refresh=true'), retry: false })
  const [settingsState, setSettingsState] = useState<ArticleSettings | null>(null); const [selectedId, setSelectedId] = useState<number | null>(null); const [title, setTitle] = useState(''); const [content, setContent] = useState('')
  useEffect(() => { if (settingsQuery.data) setSettingsState(settingsQuery.data) }, [settingsQuery.data])
  useEffect(() => { const selected = articles.data?.items.find((item) => item.id === selectedId) || articles.data?.items[0]; if (selected) { setSelectedId(selected.id); setTitle(selected.title); setContent(selected.content_markdown) } }, [articles.data?.items, selectedId])
  const selected = articles.data?.items.find((item) => item.id === selectedId)
  const generate = useMutation({ mutationFn: () => api('/api/article-settings/run-now', { method: 'POST' }), onSuccess: () => { setNotice('文章批量生成任务已启动'); queryClient.invalidateQueries({ queryKey: ['articles'] }) } })
  const saveSettings = useMutation({ mutationFn: () => api('/api/article-settings', { method: 'PUT', body: JSON.stringify(settingsState) }), onSuccess: () => { setNotice('文章计划已保存'); queryClient.invalidateQueries({ queryKey: ['article-settings'] }) } })
  const saveArticle = useMutation({ mutationFn: () => api(`/api/articles/${selectedId}`, { method: 'PATCH', body: JSON.stringify({ title, content_markdown: content, revision_note: '前端草稿箱编辑' }) }), onSuccess: () => { setNotice('文章草稿已保存'); queryClient.invalidateQueries({ queryKey: ['articles'] }) } })
  const deleteArticle = useMutation({ mutationFn: () => api(`/api/articles/${selectedId}`, { method: 'DELETE' }), onSuccess: () => { setSelectedId(null); setNotice('文章已移入回收站'); queryClient.invalidateQueries({ queryKey: ['articles'] }) } })
  const setField = <K extends keyof ArticleSettings>(key: K, value: ArticleSettings[K]) => setSettingsState((state) => state ? { ...state, [key]: value } : state)
  return <div className="page"><div className="page-head"><div><h2>公众号工作台</h2><p>系统按计划自动生成文章并保存到草稿箱；每篇文章只聚焦一个场景。</p></div><div className="actions"><Button primary onClick={() => generate.mutate()} disabled={generate.isPending}>{generate.isPending ? <LoaderCircle className="status-spin" size={15} /> : <Sparkles size={15} />}{generate.isPending ? '生成中…' : '立即生成今日文章'}</Button><ExportButton kind="scenarios" /></div></div>{(generate.error || saveSettings.error || saveArticle.error || deleteArticle.error) && <ErrorBox error={generate.error || saveSettings.error || saveArticle.error || deleteArticle.error} />}<div className="article-stats"><div><span>今日计划</span><strong>{settingsState?.daily_article_count || 1} 篇</strong></div><div><span>草稿箱</span><strong>{articles.data?.items.length || 0} 篇</strong></div><div><span>待审核</span><strong>{articles.data?.items.filter((item) => item.status === 'waiting_approval').length || 0} 篇</strong></div><div><span>图片模型</span><strong>{settingsState?.image_model_id || '未指定'}</strong></div></div><div className="article-workspace"><Panel title="自动生成计划" meta="无需手动发现主题"><div className="settings-form">{settingsState ? <><label className="switch-line"><span>启用每日自动生成</span><input type="checkbox" checked={settingsState.enabled} onChange={(e) => setField('enabled', e.target.checked)} /></label><label>生成时间<input type="time" value={settingsState.generate_time} onChange={(e) => setField('generate_time', e.target.value)} /></label><label>每天生成数量<input type="number" min="1" max="10" value={settingsState.daily_article_count} onChange={(e) => setField('daily_article_count', Number(e.target.value))} /></label><label>文章模型<select value={settingsState.article_model_id || ''} onChange={(e) => setField('article_model_id', e.target.value || null)}><option value="">跟随默认模型</option>{models.data?.items.map((item) => <option key={item.id}>{item.id}</option>)}</select></label><label>生图模型<select value={settingsState.image_model_id || ''} onChange={(e) => setField('image_model_id', e.target.value || null)}><option value="">不调用图片模型（使用 SVG 兜底）</option>{models.data?.items.map((item) => <option key={item.id}>{item.id}</option>)}</select></label><label>每篇配图数量<input type="number" min="0" max="5" value={settingsState.image_count} onChange={(e) => setField('image_count', Number(e.target.value))} /></label><label className="switch-line"><span>需要人工审核</span><input type="checkbox" checked={settingsState.approval_required} onChange={(e) => setField('approval_required', e.target.checked)} /></label><label className="switch-line"><span>自动生成封面和插图</span><input type="checkbox" checked={settingsState.auto_visuals} onChange={(e) => setField('auto_visuals', e.target.checked)} /></label><Button primary onClick={() => saveSettings.mutate()} disabled={saveSettings.isPending}>{saveSettings.isPending ? '保存中…' : '保存生成计划'}</Button></> : <Loading />}</div></Panel><Panel title="草稿箱" meta={`${articles.data?.items.length || 0} 篇`}><div className="draft-list">{articles.isLoading ? <Loading /> : articles.data?.items.length ? articles.data.items.map((item) => <button className={`draft-item ${item.id === selectedId ? 'selected' : ''}`} key={item.id} onClick={() => setSelectedId(item.id)}><span className={`status-dot ${item.status}`} /><span><strong>{item.title}</strong><small>{item.word_count} 字 · {item.status}</small></span></button>) : <div className="empty">按计划生成后，文章会出现在这里</div>}</div></Panel></div><div className="article-editor-grid"><Panel title={selected ? '编辑草稿' : '选择一篇草稿'} meta={selected ? `文章 #${selected.id}` : '草稿箱'}>{selected ? <div className="article-editor"><label>公众号标题<input value={title} onChange={(e) => setTitle(e.target.value)} /></label><label>Markdown 正文<textarea value={content} onChange={(e) => setContent(e.target.value)} /></label><div className="editor-actions"><Button primary onClick={() => saveArticle.mutate()} disabled={saveArticle.isPending}>{saveArticle.isPending ? '保存中…' : '保存草稿'}</Button><Button danger onClick={() => deleteArticle.mutate()} disabled={deleteArticle.isPending}>删除文章</Button></div></div> : <div className="empty">从左侧草稿箱选择文章开始编辑</div>}</Panel><Panel title="公众号预览" meta={selected ? `${selected.validation?.passed ? '校验通过' : '需要修改'} · ${selected.assets?.length || 0} 张图` : '实时预览'}>{selected ? <div className="article-preview">{selected.assets?.length ? <div className="article-images">{selected.assets.map((asset) => <img key={asset.id} src={`/api/articles/${selected.id}/assets/${asset.id}`} alt={asset.caption} loading="lazy" />)}</div> : null}<div dangerouslySetInnerHTML={{ __html: selected.content_html }} /></div> : <div className="empty">编辑后保存，再查看排版效果</div>}</Panel></div></div>
}
